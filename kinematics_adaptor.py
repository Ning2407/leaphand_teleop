...
class AffineMimicKinematicAdaptor(KinematicAdaptor):
    """
    支持多源线性耦合：
        q_mimic = M @ q_source + offsets
    其中：
        - q_source: 从 pinocchio qpos 里按 source_joint_names 取出的向量 (S,)
        - q_mimic:  按 mimic_joint_names 取出的向量 (M,)
        - M: (M, S) 的矩阵
        - offsets: (M,) 的偏置

    这个适配器会在 forward_qpos 中把 mimic 关节写回 pin_qpos；
    在 backward_jacobian 中，把 mimic 关节的雅可比“回流”到各个 source 关节（链式法则）。
    """
    def __init__(
        self,
        robot: RobotWrapper,
        ...
        source_joint_names: List[str],
        mimic_joint_names: List[str],
        multipliers_matrix: np.ndarray,   # 形状 (len(mimic), len(source))
        offsets: List[float],
    ):
        super().__init__(robot, target_joint_names)

        self.offsets = np.asarray(offsets, dtype=float)
        self.M = np.asarray(multipliers_matrix, dtype=float)
        ...
        S = len(self.idx_pin2source)
        M = len(self.idx_pin2mimic)
        if self.M.shape != (M, S):
            raise ValueError(
                f"multipliers_matrix shape mismatch. "
                f"Expect ({M}, {S}), got {self.M.shape}"
            )
        if self.offsets.shape != (M,):
            raise ValueError(
                f"offsets length mismatch. Expect {M}, got {self.offsets.shape[0]}"
            )

        if len(set(mimic_joint_names).intersection(set(target_joint_names))) > 0:
            raise ValueError(
                "Mimic joints must not be part of target joints. "
                "Please remove them from target_joint_names."
            )

    def forward_qpos(self, pin_qpos: np.ndarray) -> np.ndarray:
        """
        将多源线性关系写回到 pin_qpos 的 mimic 关节上
        """
        q_source = pin_qpos[self.idx_pin2source]          # (S,)
        q_mimic = self.M @ q_source + self.offsets        # (M,)
        pin_qpos[self.idx_pin2mimic] = q_mimic
        return pin_qpos

    def backward_jacobian(self, jacobian: np.ndarray) -> np.ndarray:
        """
        链式法则：
          J_target[..., source_k] += sum_i ( J[..., mimic_i] * M[i, k] )
        同时保留 target joints 自身对应列
        """
        # 1) 先取出 target 的雅可比（会被我们原地加和）
        J_t = jacobian[..., self.idx_pin2target]                  # (..., T)
        # 2) 取出 mimic 的雅可比
        J_mimic = jacobian[..., self.idx_pin2mimic]               # (..., M)

        # 3) 把 mimic 对 source 的影响回流到对应的 target-source 列
        #    对于每个 source 关节 k ：J_t[..., idx_target2source[k]] += sum_i J_mimic[..., i] * M[i, k]
        for k, tgt_col in enumerate(self.idx_target2source):
            # 组合所有 mimic 列的线性组合
            combo = np.tensordot(J_mimic, self.M[:, k], axes=([J_mimic.ndim - 1], [0]))
            # tensordot 结果维度与 J_t[..., tgt_col] 对齐
            J_t[..., tgt_col] += combo
